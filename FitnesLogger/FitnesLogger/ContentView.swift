...
struct ExerciseEditorSection: View {
    ...
    private var isCardio: Bool {
        ...
        || exercise.name.lowercased().contains("bike")
        || exercise.name.lowercased().contains("cardio")
    }
    var body: some View {
        Section {
            HStack {
...
struct SetRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            if isCardio {
                Stepper("Minutes: \(set.reps)", value: $set.reps, in: 1...300)
            } else {
...
struct AddWorkoutView: View {
    ...
    // Enoten sheet presenter
    private enum ActiveSheet: Identifiable {
        case addExercise
        case changeExercise(UUID)

        var id: String {
            switch self {
            case .addExercise: return "addExercise"
            case .changeExercise(let id): return "change-\(id.uuidString)"
            }
        }
    }
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                ...
                                Button(exercise.name.isEmpty ? "Choose" : "Change") {
                                    activeSheet = .changeExercise(exercise.id)
                                }
                                .font(.caption)
                            ...
                            Button("Add set") {
                                exercise.sets.append(WorkoutSet(weightKg: 0, reps: 8))
                            }
                            .font(.caption)
                            .buttonStyle(.borderless)
                        }
        ...
        .sheet(item: $activeSheet, onDismiss: {
            ...
            ExercisePickerSheet { picked in
                ...
                case .changeExercise(let exID):
                    if let idx = exercises.firstIndex(where: { $0.id == exID }) {
